...
{		
	...
	{		
		$sql="select member_id from all24vote where member_id=".$_SESSION["mem"]["member_id"]." and contest_id=".$_POST["contest_id"];
		$g=new general();
		$fv=$g->getSQL($sql,0);		
		if(sizeof($fv)>0)
		{
	...
	foreach ($recordSet as $re)
		{
			...
			echo "<contest_title>".$re["contest_title"]."</contest_title>";
			echo "<contest_desc>".$re["contest_desc"]."</contest_desc>";
			echo "<contest_date>".$re["contest_date"]."</contest_date>";
			
			echo "<totalVote>".$re["totalVote"]."</totalVote>";
			echo "<member_user>".$re["member_user"]."</member_user>";
